...
def furlong_hu(f):
    """A "7f" senkinek nem mond semmit. Kiirjuk, es hozzatesszuk
    a metert is - 1 furlong = 201 meter."""
    return f"{f} furlong ({round(f * 201 / 100) * 100} m)"


def surface_note(surface_hu):
    return {'homok': 'homokpálya', 'gyep': 'gyeppálya',
            'szintetikus': 'szintetikus pálya'}.get(surface_hu, surface_hu)
...
def milestones(sid):
    """MI FELE HALADOK? — a mérföldkövek MAR LETEZNEK a motorban,
    csak a jatekos nem latta oket.

    Nem diszites, hanem TERKEP: megmutatja, mit lehet elerni es
    milyen messze vagy.
    """
    st = WORLDS[sid]
    world, player, db = st['world'], st['player'], st['world'].db
    mine = [h for h in world.horses.values()
            if h.owner_id == player or h.breeder_id == player]
    bred = [h for h in world.horses.values() if h.breeder_id == player]

    tot_wins = sum(db.stats(h.horse_id).wins for h in mine)
    tot_bt = sum(db.stats(h.horse_id).black_type_wins for h in mine)
    tot_classic = sum(db.stats(h.horse_id).classic_wins for h in mine)
    bred_wins = sum(db.stats(h.horse_id).wins for h in bred
                    if h.sire_id is not None)
    bred_bt = sum(db.stats(h.horse_id).black_type_wins for h in bred
                  if h.sire_id is not None)

    # blue hen: harom black type utod ugyanattol a kancatol
    best_hen, best_hen_name = 0, None
    for h in world.owned_by(player, 'breeding'):
        n = db.stats(h.horse_id).progeny_black_type
        if n > best_hen:
            best_hen, best_hen_name = n, h.name

    # globalis menlista: black type + B+ index
    listable = [h for h in world.owned_by(player)
                if h.sex == 'colt'
                and db.stats(h.horse_id).black_type_wins > 0
                and ST.grade_rank(h.grade()) >= ST.grade_rank('B+')]

    def ms(key, title, done, progress=None, target=None, hint=None):
        return {'key': key, 'title': title, 'done': bool(done),
                'progress': progress, 'target': target, 'hint': hint}

    return [
        ms('first_win', 'Első győzelem', tot_wins > 0,
           tot_wins, 1, 'Nevezz egy lovat a saját nyereménysávjába.'),
        ms('bred_win', 'Első saját tenyésztésű győztes', bred_wins > 0,
           bred_wins, 1, 'A csikód 2 évesen állhat versenybe.'),
        ms('black_type', 'Első black type győzelem', tot_bt > 0,
           tot_bt, 1, 'Group futamhoz 60 000 B$ életnyeremény kell.'),
        ms('bred_bt', 'Saját tenyésztésű black type győztes', bred_bt > 0,
           bred_bt, 1, 'Ez teszi a ménjeidet eladhatóvá.'),
        ms('classic', 'Klasszikus győzelem', tot_classic > 0,
           tot_classic, 1, 'A csúcs. 300 000 B$ nyeremény a belépő.'),
        ms('stud_listed', 'Mén a globális listán', bool(listable),
           len(listable), 1,
           'Black type győzelem ÉS legalább B+ tenyészindex kell.'),
        ms('blue_hen', 'Blue hen kanca', best_hen >= 3,
           best_hen, 3,
           f"{best_hen_name}: {best_hen}/3 black type utód"
           if best_hen_name else 'Három black type utód ugyanattól a kancától.'),
    ]


def todo_summary(sid):
    """MI A DOLGOM MOST? — ez volt a legnagyobb hiany.

    A jatekos ne panelokat bongesszen, hanem lassa, mi var rea.
    """
    st = WORLDS[sid]
    world, cal, acal = st['world'], st['calendar'], st['auctions']
    player, clock = st['player'], st['clock']
    items = []

    racers = world.owned_by(player, 'racer')
    mares = [m for m in world.owned_by(player, 'breeding')
             if m.sex == 'filly' and (m.breeding_bar or 0) > 0]

    # --- nevezes nelkuli, friss lovak ---
    unentered = []
    for h in racers:
        if not cal.entries_of(h.horse_id) and LC.freshness_ready(h.freshness):
            unentered.append(h.name)
    if unentered:
        items.append({
            'tab': 'race', 'urgency': 'high',
            'title': f"{len(unentered)} ló nincs nevezve",
            'detail': ', '.join(unentered[:3]) +
                      ('…' if len(unentered) > 3 else ''),
            'action': 'Versenynaptár',
        })

    # --- pihenore szorulo lovak ---
    tired = [h.name for h in racers if not LC.freshness_ready(h.freshness)]
    if tired:
        items.append({
            'tab': 'race', 'urgency': 'low',
            'title': f"{len(tired)} ló pihenőre szorul",
            'detail': ', '.join(tired[:3]),
            'action': 'Léptesd az időt',
        })

    # --- fedeztetheto kancak ---
    unplanned = [m.name for m in mares
                 if getattr(m, 'last_covered_season', None) != world.season]
    if unplanned:
        items.append({
            'tab': 'breed', 'urgency': 'high',
            'title': f"{len(unplanned)} kanca fedeztethető",
            'detail': ', '.join(unplanned[:3]),
            'action': 'Fedeztetési terv',
        })

    # --- allatorvosi felmeres nelkuli kancak ---
    novet = [m.name for m in mares if getattr(m, 'health_grade', None) is None]
    if novet:
        items.append({
            'tab': 'breed', 'urgency': 'low',
            'title': f"{len(novet)} kancának nincs állatorvosi felmérése",
            'detail': 'A válogatós mének elvárják',
            'action': 'Fedeztetési terv',
        })

    # --- kozelgo aukcio ---
    soon = [s for s in acal.open_catalogues()
            if 0 < s.starts_at - clock.now() < 2880]
    if soon:
        items.append({
            'tab': 'market', 'urgency': 'normal',
            'title': f"{len(soon)} aukció két napon belül",
            'detail': soon[0].house_name + ' · ' + soon[0].sale_label,
            'action': 'Aukciók',
        })

    # --- ferohely ---
    active = [h for h in world.owned_by(player)
              if h.stage not in ('retired_out', 'pensioned')]
    cap = FA.stable_capacity(st['farm']['stable_level'])
    if len(active) >= cap:
        items.append({
            'tab': 'market', 'urgency': 'high',
            'title': 'Betelt az istálló',
            'detail': f'{len(active)}/{cap} férőhely — adj el vagy bővíts',
            'action': 'Aukciók',
        })

    order = {'high': 0, 'normal': 1, 'low': 2}
    items.sort(key=lambda i: order.get(i['urgency'], 3))
    return items
    ...
    if session_id and session_id in acal.sessions:
        ...
        for lot in sess.lots:
            ...
            lots.append({
                ...
                'age_label': f"{h.age} éves",
                'sex_label': 'kanca' if h.sex == 'filly' else 'mén',
                'stage': h.stage,
                'stage_label': {'yearling': 'yearling', 'racer': 'versenyló',
                                'breeding': 'tenyészállat',
                                'foal': 'csikó'}.get(h.stage, h.stage),
...
def race_calendar(sid, horse_id=None):
    ...
    def race_row(r):
        return {
            ...
            'distance_label': furlong_hu(r.distance_f),
            'surface': r.surface_hu,
            'surface_label': surface_note(r.surface_hu),
            'band': r.band,
    ...
    return {
        ...
        'racers': [{'horse_id': h.horse_id, 'name': h.name,
                    ...
                    'policy': G.run_policy(h),
                    'policy_label': G.RUN_POLICIES[G.run_policy(h)],
                    'entries': len(cal.entries_of(h.horse_id))}
                   for h in world.owned_by(st['player'], 'racer')],
    }
...
def set_policy(sid, horse_id, policy):
    st = WORLDS[sid]
    h = st['world'].horses.get(horse_id)
    if h is None:
        return {'ok': False, 'reason': 'Nincs ilyen ló'}
    if not G.set_run_policy(h, policy):
        return {'ok': False, 'reason': 'Ismeretlen beállítás'}
    return {'ok': True, 'policy': policy,
            'label': G.RUN_POLICIES[policy]}
...
def advance_time(sid, minutes):
    ...
    # --- AUTOMATIKUS NEVEZES az ido leptetese ELOTT ---
    # Igy nem kell minden lovat kezzel nevezni. A 'manual' politikaju
    # lovak erintetlenek maradnak.
    auto = G.auto_enter_all(world, cal, st['player'])
    for h, r in auto:
        world.ev('entry', f"{h.name} automatikusan nevezve: {r.track_name} "
                          f"{r.distance_f} furlong, {r.bracket}")
...
def serialise(sid):
    ...
    return {
        'session_id': sid,
        'now': s['clock'].label(),
        'todo': todo_summary(sid),
        'milestones': milestones(sid),
        'season': world.season,
        'money': int(s['money']),
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if p == '/api/policy':
            ...
            if sid not in WORLDS:
                return self._send({'error': 'Nincs ilyen munkamenet'}, 404)
            return self._send(set_policy(sid, data.get('horse_id'),
                                         data.get('policy')))
