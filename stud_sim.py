...
# Enyhe kor-alapu konyvcsokkenes. A JATEKOS DONTESE szerint ez NEM a fo
# kiesesi mechanizmus - azt a kereslet (lasd 5. blokk) intezi. Ez csak
# egy finom, elettani jellegu taper nagyon idos korban.
AGE_BOOK_TAPER = [
    (18, 0.85),   # 18 evtol a konyv 85%-a
    (21, 0.65),
    (24, 0.45),
]


def get_season_book(stud_index, age):
    """A men adott szezonban elerheto MAXIMALIS konyvmeretet adja
    (a tenyleges kihasznaltsagot a kereslet donti el, lasd lentebb)."""
    base = STUD_TIERS[stud_index]['book']
    factor = 1.0
    for threshold, f in AGE_BOOK_TAPER:
        if age >= threshold:
            factor = f
...
if __name__ == '__main__':
    ...
    print("--- 2) KOR-ALAPU ENYHE KONYVCSOKKENES ---")
    for age in [4, 10, 17, 18, 21, 24]:
        print(f"  A+ mén, {age:2d} éves -> könyv: {get_season_book('A+', age):3d}")
    print("  (a fő kiesési mechanizmus NEM ez, hanem a kereslet — lásd 5. blokk)\n")
