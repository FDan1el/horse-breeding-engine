...
# =======================================================================
# 6) TAKARMANY-PIACTER: SZAKASZOS (STAGE-LOCKED) BONUSZ-RENDSZER
# =======================================================================
# A JATEKOS KONCEPCIOJA: a takarmanyozas egy piacter-mechanika. Az alap-
# alapanyagok (szena, zab, arpa) NEM markasak, csak egy gazda/termelo
# nevet es minosegi szintet kapnak. A koncentraltapok MARKASAK,
# kategorizaltak, es a jatekos egyszerre TOBBET is valaszthat. Az
# adagolast a "virtualis tréner" automatikusan kezeli.
#
# SZAKASZ-LEZARAS (STAGE LOCKING): mivel a lo BARMELYIK eletszakaszban
# gazdat cserelhet (yearling-arveres, NPC- vagy jatekos-eladas), minden
# szakasz a lezarasakor VEGLEGESEN rogzul. Amit egy szakaszban elmulaszt
# a tulajdonos, azt a kesobbi tulajdonos MAR NEM tudja potolni.
#
# A 15%-os felnott-takarmany sav ket szakaszra bomlik:
#
#   1. SZAKASZ - CSIKOKOR (0-1 ev):     max 9%
#   2. SZAKASZ - YEARLINGKOR (1-2 ev):  max 6%
#
# A 9/6 ARANY VALOS NOVEKEDESI ADATBOL SZAMOLT (nem onkenyes):
# Ot fuggetlen forras novekedesi gorbeje alapjan a szuletes->1 ev es az
# 1 ev->2 ev kozotti testtomeg-gyarapodas aranya atlagosan 61.4% / 38.6%:
#   - Kentucky Equine Research: 6 ho=40% / 12 ho=60% / 24 ho=96% kifejlett
#     testtomeg -> 1. ev 50pp, 2. ev 36pp (58/42)
#     https://www.ker.com/equinews/nature-versus-nurture-factors-affect-growth-horses/
#   - Oklahoma State University Extension: 12 ho=50-60%, 24 ho=80-90%
#     -> (56/44)
#     https://extension.okstate.edu/fact-sheets/managing-young-horses-for-sound-growth
#   - Purina Animal Nutrition: 12 ho=65-70% kifejlett testtomeg -> (66/34)
#     https://www.purinamills.com/horse-feed/education/detail/proper-foal-feeding-through-the-first-year
#   - Evans (1990) adatai: 6/12/18 ho = 46%/67%/79% -> (71/29)
#     https://starmilling.com/estimating-body-weight-of-growing-foals/
#   - Merck Animal Health: 6 ho=50% testtomeg, 80% marmagassag -> (55/45)
#     https://www.merck-animal-health-usa.com/horse-owners-and-professionals/foal-mare-care/foal-care-overview/foal-nutrition-and-foal-growth/
# Atlag: 61.4/38.6 -> 15%-bol 9.2% / 5.8% -> KEREKITVE 9% / 6%.
#
# MINOSEGI MEGEROSITES (miert indokolt, hogy az 1. ev sulyosabb):
#   - Az elso 6 honapban robbanasszeru a novekedes, az izuletek
#     serulekenyek, es a taplalasi hiany VAGY tulzas fejlodesi
#     csontrendellenesseghez (DOD) vezethet.
#   - A labszarcsont/csud meretei a valasztastol 3 eves korig kevesebb,
#     mint 7%-kal nonek - az also vegtag lenyegeben KESZ 1 eves korra.
#     https://www.fei.org/stories/lifestyle/teach-me/how-big-will-your-foal-or-youngster-get
#   Azaz: amit az 1. evben elmulasztanak, az fizikailag sem potolhato -
#   ez pontosan alatamasztja a szakasz-lezaras jatekmechanikat.
#
# SZAKASZONKENTI BONTAS ("A" valtozat - egyszeru, atlathato, minden
# komponens EGY szakaszon belul marad, nem log at a szakaszhataron):
#   1. szakasz (9%):  alap-alapanyag 3 + csikotap 3 + vitaminos 3
#   2. szakasz (6%):  alap-alapanyag 2 + yearling-tap 2 + vitaminos 2

# --- 1. SZAKASZ: CSIKOKOR (0-1 ev), max 9% ---
FOAL_STAGE_MAX_PCT = 9.0
FOAL_BASE_QUALITY_BONUS = {   # szena/zab minoseg
    'gyenge': 0.0,
    'kozepes': 1.2,
    'jo': 2.1,
    'kivalo': 3.0,
}
FOAL_CONCENTRATE_BONUS = 3.0   # csikotap (creep feed) hasznalata
FOAL_VITAMIN_BONUS = 3.0       # vitaminos kiegeszito

# --- 2. SZAKASZ: YEARLINGKOR (1-2 ev), max 6% ---
YEARLING_STAGE_MAX_PCT = 6.0
YEARLING_BASE_QUALITY_BONUS = {
    'gyenge': 0.0,
    'kozepes': 0.8,
    'jo': 1.4,
    'kivalo': 2.0,
}
YEARLING_CONCENTRATE_BONUS = 2.0   # yearling/novendek-tap
YEARLING_VITAMIN_BONUS = 2.0       # vitaminos kiegeszito

MAX_TOTAL_FEED_BONUS_PCT = FOAL_STAGE_MAX_PCT + YEARLING_STAGE_MAX_PCT  # = 15.0


def calculate_foal_stage_bonus_pct(base_feed_quality, uses_concentrate, uses_vitamin):
    """1. SZAKASZ (csikokor, 0-1 ev): max 9%.
    A szakasz lezarasakor VEGLEGESEN rogzul - kesobbi tulajdonos nem
    tudja potolni."""
    bonus = FOAL_BASE_QUALITY_BONUS.get(base_feed_quality, 0.0)
    if uses_concentrate:
        bonus += FOAL_CONCENTRATE_BONUS
    if uses_vitamin:
        bonus += FOAL_VITAMIN_BONUS
    return round(min(bonus, FOAL_STAGE_MAX_PCT), 2)


def calculate_yearling_stage_bonus_pct(base_feed_quality, uses_concentrate, uses_vitamin):
    """2. SZAKASZ (yearlingkor, 1-2 ev): max 6%.
    A szakasz lezarasakor VEGLEGESEN rogzul."""
    bonus = YEARLING_BASE_QUALITY_BONUS.get(base_feed_quality, 0.0)
    if uses_concentrate:
        bonus += YEARLING_CONCENTRATE_BONUS
    if uses_vitamin:
        bonus += YEARLING_VITAMIN_BONUS
    return round(min(bonus, YEARLING_STAGE_MAX_PCT), 2)


def calculate_feed_bonus_pct(foal_stage_pct, yearling_stage_pct):
    """A ket lezart szakasz osszege (max 15%). A szakaszok kulon-kulon
    rogzulnek, itt csak osszeadodnak."""
    return round(min(foal_stage_pct + yearling_stage_pct, MAX_TOTAL_FEED_BONUS_PCT), 2)
...
if __name__ == '__main__':
    ...
    # --- 6) Szakaszos (stage-locked) takarmany-bonusz validacio ---
    print("--- 6) SZAKASZOS TAKARMANY-BONUSZ VALIDACIO (9% csikokor + 6% yearlingkor) ---")
    print("  A 9/6 arany ot fuggetlen novekedesi-gorbe forras atlagabol (61.4/38.6) szarmazik.\n")

    print("  1. SZAKASZ - CSIKOKOR (0-1 ev), max 9%:")
    foal_cases = [
        ('gyenge', False, False, 0.0, "elhanyagolt csiko"),
        ('kivalo', False, False, 3.0, "kivalo szena/zab onmagaban"),
        ('jo', True, False, 5.1, "jo alap (2.1) + csikotap (3.0)"),
        ('kivalo', True, True, 9.0, "MAXIMUM: kivalo alap (3) + csikotap (3) + vitamin (3)"),
    ]
    stage_ok = True
    for quality, conc, vit, expected, note in foal_cases:
        result = calculate_foal_stage_bonus_pct(quality, conc, vit)
        status = "OK" if abs(result - expected) < 0.01 else "ELTERES"
        if status == "ELTERES":
            stage_ok = False
        print(f"    {quality:8s} tap={'i' if conc else 'n'} vit={'i' if vit else 'n'} -> {result:5.2f}%  (varhato {expected:5.2f}%)  [{status}]  {note}")

    print("\n  2. SZAKASZ - YEARLINGKOR (1-2 ev), max 6%:")
    yearling_cases = [
        ('gyenge', False, False, 0.0, "elhanyagolt yearling"),
        ('kivalo', False, False, 2.0, "kivalo szena/zab onmagaban"),
        ('jo', True, False, 3.4, "jo alap (1.4) + yearling-tap (2.0)"),
        ('kivalo', True, True, 6.0, "MAXIMUM: kivalo alap (2) + yearling-tap (2) + vitamin (2)"),
    ]
    for quality, conc, vit, expected, note in yearling_cases:
        result = calculate_yearling_stage_bonus_pct(quality, conc, vit)
        status = "OK" if abs(result - expected) < 0.01 else "ELTERES"
        if status == "ELTERES":
            stage_ok = False
        print(f"    {quality:8s} tap={'i' if conc else 'n'} vit={'i' if vit else 'n'} -> {result:5.2f}%  (varhato {expected:5.2f}%)  [{status}]  {note}")

    print("\n  SZAKASZ-LEZARAS DEMONSTRACIO (mi tortenik gazdacsere eseten):")
    scenarios = [
        (9.0, 6.0, "vegig gondos tulajdonos"),
        (9.0, 0.0, "kivaloan nevelt csiko, majd elhanyagolt yearling (eladas utan)"),
        (0.0, 6.0, "elhanyagolt csiko, majd gondos uj tulajdonos - NEM tudja potolni"),
        (0.0, 0.0, "vegig elhanyagolt"),
    ]
    for foal_pct, yearling_pct, note in scenarios:
        total = calculate_feed_bonus_pct(foal_pct, yearling_pct)
        print(f"    csikokor {foal_pct:4.1f}% + yearlingkor {yearling_pct:4.1f}% = {total:5.2f}%  ({note})")

    max_feed = calculate_feed_bonus_pct(FOAL_STAGE_MAX_PCT, YEARLING_STAGE_MAX_PCT)
    bonus_ok = stage_ok and abs(max_feed - 15.0) < 0.01
    print(f"\n  A ket szakasz maximuma pontosan 15%: {max_feed:.2f}%  [{'OK' if bonus_ok else 'HIBA'}]\n")
