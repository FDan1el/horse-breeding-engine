...
CREATE TABLE race (
    race_id      bigint GENERATED ALWAYS AS IDENTITY,
    ...
    -- A PostgreSQL megköveteli, hogy particionált táblán az elsődleges
    -- kulcs TARTALMAZZA a particionáló oszlopot. Ezért (race_id, season).
...
CREATE TABLE race_result (
    ...
    -- A season DENORMALIZÁLVA ide is: így a race_result UGYANÚGY
    -- particionálható, mint a race, és a partíciók egymáshoz igazodnak.
    -- Enélkül a két tábla join-ja partíciók között szóródna szét.
    season      integer NOT NULL,
    ...
    PRIMARY KEY (race_id, horse_id, season),
    FOREIGN KEY (race_id, season) REFERENCES race(race_id, season)
) PARTITION BY RANGE (season);
...
-- PARTÍCIÓK
-- ---------------------------------------------------------------------
-- Szezononként egy-egy: a régi szezonok hidegtárba tolhatók, a
-- lekérdezések többsége az aktuális szezont érinti.
--
-- A DEFAULT partíció biztonsági háló: ha egy új szezon partíciója
-- lemarad, a beszúrás nem száll el, hanem ide kerül. Éles üzemben
-- érdemes figyelni, hogy ne kerüljön bele semmi.

CREATE TABLE race_s01 PARTITION OF race FOR VALUES FROM (1) TO (2);
CREATE TABLE race_s02 PARTITION OF race FOR VALUES FROM (2) TO (3);
CREATE TABLE race_s03 PARTITION OF race FOR VALUES FROM (3) TO (4);
CREATE TABLE race_s04 PARTITION OF race FOR VALUES FROM (4) TO (5);
CREATE TABLE race_s05 PARTITION OF race FOR VALUES FROM (5) TO (6);
CREATE TABLE race_s06 PARTITION OF race FOR VALUES FROM (6) TO (7);
CREATE TABLE race_s07 PARTITION OF race FOR VALUES FROM (7) TO (8);
CREATE TABLE race_s08 PARTITION OF race FOR VALUES FROM (8) TO (9);
CREATE TABLE race_default PARTITION OF race DEFAULT;

CREATE TABLE race_result_s01 PARTITION OF race_result FOR VALUES FROM (1) TO (2);
CREATE TABLE race_result_s02 PARTITION OF race_result FOR VALUES FROM (2) TO (3);
CREATE TABLE race_result_s03 PARTITION OF race_result FOR VALUES FROM (3) TO (4);
CREATE TABLE race_result_s04 PARTITION OF race_result FOR VALUES FROM (4) TO (5);
CREATE TABLE race_result_s05 PARTITION OF race_result FOR VALUES FROM (5) TO (6);
CREATE TABLE race_result_s06 PARTITION OF race_result FOR VALUES FROM (6) TO (7);
CREATE TABLE race_result_s07 PARTITION OF race_result FOR VALUES FROM (7) TO (8);
CREATE TABLE race_result_s08 PARTITION OF race_result FOR VALUES FROM (8) TO (9);
CREATE TABLE race_result_default PARTITION OF race_result DEFAULT;
