...
class RaceCalendar:
    ...
    def entries_of(self, horse_id, upcoming_only=True):
        """A lo JOVOBELI nevezesei.

        HIBA VOLT: a mar LEFUTOTT nevezeseket is visszaadta, ezert az
        automatikus nevezes ket futam utan azt hitte, betelt a keret -
        a lovak soha tobbet nem neveztek.
        """
        out = []
        for rid in self.by_horse.get(horse_id, []):
            ...
            if r is None:
                continue
            if upcoming_only and r.status in (RaceStatus.RUN,
