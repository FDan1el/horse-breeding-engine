...
# A vilag felepitese ~0.6 mp, de minden uj jatekhoz ujra kellene.
# Ezert egy ALAPITO VILAGOT gyorsitotarazunk, es minden jatekos ennek
# egy MASOLATABOL indul. Igy a kozos vilag koherens marad.
_WORLD_CACHE = {}


def _base_world(seed):
    if seed not in _WORLD_CACHE:
        if len(_WORLD_CACHE) > 3:
            _WORLD_CACHE.clear()
        _WORLD_CACHE[seed] = WG.build_world(seed=seed)
    return _WORLD_CACHE[seed]


def new_game(feed_quality='jo', trainer_level='mid', seed=None):
    """Uj jatek AZ ALAPITO VILAGBOL.

    A kezdo lovaknak VALODI szarmazasuk van, a menlista valodi lovakbol
    all pedigrevel es versenymulttal, es a verseny-ellenfelek sem
    szintetikusak.
    """
    seed = seed if seed is not None else random.randint(1, 10**9)
    world, generations = WG.build_world(seed=seed)
    player = 'player'
