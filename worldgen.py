...
def build_world(config=None, seed=None, verbose=False):
    ...
    for gen in range(1, cfg['generations'] + 1):
        ...
        # a generacio LEFUT
        for h in new_horses:
            # A KARRIERHOSSZ VALTOZO, nem fix.
            #
            # HIBA VOLT: minden tortenelmi lo pontosan 22-t futott,
            # ezert mindegyik pontosan ugyanannyi idos lett.
            # A valos palyafutasok eltéroek: van, aki 4 start utan
            # abbahagyja, van, aki 40-et fut.
            n = max(2, round(rng.gauss(cfg['races_per_horse'] * 0.7,
                                       cfg['races_per_horse'] * 0.45)))
            race_history(world, h, trainer, jockey, min(n, 44),
                         cohort=new_horses)
        ...
        gen_mares, gen_studs = [], []
        for h in new_horses:
            # A KOR A GENERACIOBOL SZARMAZIK, nem fix 6.
            #
            # HIBA VOLT: minden lo pontosan 6 eves lett, igy az
            # aukcios katalogusban is minden ugyanannyi idos volt.
            # A generaciok idoben egymas utan jonnek: mire a kovetkezo
            # megszuletik, az elozo mar ~4-5 evvel idosebb.
            h.birth_generation = gen
            h.age = 4 + rng.randint(0, 2)
    ...
    # --- A KOROK VISSZAMENOLEG BEALLITASA ---
    # A LEGUTOLSO generacio a "jelen": fiatal lovak. A korabbiak
    # annyival idosebbek, ahany generacio eltelt azota.
    YEARS_PER_GENERATION = 4
    last_gen = cfg['generations']
    for h in world.horses.values():
        bg = getattr(h, 'birth_generation', 0)
        base = 3 + (last_gen - bg) * YEARS_PER_GENERATION
        # SZORAS a generacion belul is - kulonben savosan tomorulnek
        # a korok (4-6, 9-11, 14-16...), ami mesterségesnek latszik.
        age = round(rng.gauss(base + 1.5, 2.2))

        # A KOR NEM LEHET KEVESEBB, MINT AMIT A VERSENYMULT INDOKOL.
        #
        # HIBA VOLT: egy 2 eves lo 22 starttal szerepelt a menlistan.
        # A lo 2 evesen all versenybe, es szezononkent ~6-7 startot fut
        # (season_sim.py), tehat a minimum kor ebbol szamolhato.
        st = world.db.stats(h.horse_id)
        min_age = 2 + max(0, (st.starts - 1) // 7)
        h.age = max(min_age, age)

    # --- A SZULO SOSEM LEHET FIATALABB A GYEREKENEL ---
    #
    # AZ AUDIT DERITETTE KI: a generacios alapkor koré tett szoras
    # (+/-2.2 ev) nagyobb volt, mint a generaciok kozotti tavolsag
    # (4 ev), ezert 55 esetben a szulo fiatalabb lett a gyerekenel.
    #
    # Generaciorol generaciora haladva javitjuk: minden szulo legalabb
    # 4 evvel idosebb a legidosebb gyerekenel.
    for _ in range(cfg['generations'] + 1):
        for h in world.horses.values():
            for key in ('sire_id', 'dam_id'):
                p = world.horses.get(getattr(h, key, None))
                if p is not None and p.age < h.age + 4:
                    p.age = h.age + 4


    # --- A KORHOZ IGAZODO STATUSZ (a korjavitas UTAN) ---
    for h in world.horses.values():
        if h.age > 22:
            h.stage = 'retired_out'
        elif h.age > 17:
            h.stage = 'pensioned'
        elif h.age > 9 and h.stage == 'racer':
            h.stage = 'breeding'
            if h.sex == 'filly' and h.breeding_bar is None:
                h.breeding_bar = LC.breeding_bar_from_career(h.career_used)

    # --- AKTIV VERSENYLOVAK A VILAGBAN ---
    #
    # HIBA VOLT: a generalas vegen MINDEN lo tenyesztesbe vagy
    # nyugdijba kerult, igy egyetlen aktiv versenylo sem maradt -
    # a mezonyok megint szintetikusak lettek volna.
    #
    # A legfiatalabb generacio versenykorban van: ok adjak az
    # ellenfeleket a jatekos lovainak.
    young = [h for h in world.horses.values()
             if 2 <= h.age <= 8 and h.stage in ('breeding', 'racer')
             and h.career_bar > 15]
    young.sort(key=lambda h: h.age)
    for h in young[:max(40, len(young) // 2)]:
        h.stage = 'racer'
        h.freshness = 100.0
...
def give_starting_stock(world, generations, player_id, feed_quality='jo'):
    ...
    for _ in range(2):
        ...
        foal.sex = 'colt'
        foal.stage = 'racer'
        # A KEZDO VERSENYLO 3 EVES, de a szuleinek legalabb 4 evvel
        # idosebbnek kell lenniuk. Ha nem azok, felhuzzuk oket -
        # kulonben az utod idosebb lesz a szulojenel.
        foal.age = 3
        for parent in (sire, dam):
            if parent.age < foal.age + 4:
                parent.age = foal.age + 4
        foal.birth_generation = generations and len(generations) or 1
