...
def age_up(world: World, feed_quality='jo', conc=True, vit=True):
    ...
    for h in list(world.horses.values()):
        # MINDEN lo oregszik, a kikerultek is.
        #
        # AZ AUDIT DERITETTE KI: a kikerult lovak megalltak a korukban,
        # mikozben az utodaik tovabb oregedtek - 12 szezon utan az
        # utod IDOSEBB lett a szulojenel (121 eset). A kor a
        # SZULETESTOL szamit, nem az aktivitastol.
        h.age += 1
        if h.stage == 'retired_out':
            continue

        # AZ AUDIT DERITETTE KI: a vilag NPC-lovai oregedtek, de sosem
        # kerultek ki - 12 szezon utan 33 eves lovak szerepeltek a
        # menlistan. A kikerules MINDEN lora vonatkozik, nem csak a
        # jatekoseira.
        if h.age > 24:
            h.stage = 'retired_out'
            continue
        if h.age > 18 and h.stage in ('racer', 'breeding'):
            h.stage = 'pensioned'
            continue
        if h.age > 11 and h.stage == 'racer':
            h.stage = 'breeding'
...
# =======================================================================
# 5c) FUTTATASI POLITIKA
# =======================================================================
# A menedzserjatekok bevett logikaja: a jatekos ELVET allit be, a
# szimulacio vegrehajtja. Aki akar, kezzel nevez; aki nem, beallitja
# es lepteti az idot.
#
# Ez valtja ki a "Szezon lefuttatasa" gombot: nem ket kulon idorendszer
# lesz, hanem EGY - es a nevezes automatizalhato.
RUN_POLICIES = {
    'manual': 'Kézzel nevezem',
    'auto': 'Automatikus — pihenten, a legjobb elérhető futamba',
    'rest': 'Pihentetem',
}
DEFAULT_RUN_POLICY = 'auto'


def run_policy(horse):
    return getattr(horse, 'run_policy', DEFAULT_RUN_POLICY)


def set_run_policy(horse, policy):
    if policy in RUN_POLICIES:
        horse.run_policy = policy
        return True
    return False


def auto_enter(world, calendar, horse, owner_id, max_entries=2):
    """Automatikus nevezes a lo politikaja szerint.

    A "legjobb elérhető futam" = a legmagasabb dijazasu, amire a lo
    jogosult, es ahol meg van hely. A frissesseget figyelembe veszi:
    ha a lo pihenore szorul, NEM nevez.
    """
    if run_policy(horse) != 'auto':
        return []
    if not LC.freshness_ready(horse.freshness):
        return []
    if horse.career_bar <= 0 or horse.life_bar <= 0:
    ...
    already = len(calendar.entries_of(horse.horse_id))
    if already >= max_entries:
        return []

    options = calendar.open_races(horse, limit=60)
    if not options:
        return []

    # A "LEGJOBB ELERHETO FUTAM" NEM a legnagyobb dijazasu.
    #
    # HIBA VOLT: a dij dominalta a dontest, ezert a kezdo lovak a
    # 182 000-es nyilt futamokba nevezte, ahol eselyuk sem volt -
    # 8. es 6. hely, nulla nyeremeny.
    #
    # A helyes valasztas az a futam, ahol a lo VERSENYKEPES: a sajat
    # nyeremenysavja, jo tavillesztessel. A dij csak ezen belul dont.
    own_bracket = TK.bracket_for_earnings(horse.career_earnings,
                                          horse.wins > 0)

    def score(r):
        # 1. a sajat sav a legfontosabb
        same = 1.0 if r.bracket == own_bracket['label'] else 0.25
        # 2. tavillesztes (race_sim.py: 2.07x kulonbseget jelent)
        apt = horse.genetics.get(r.band, 50)
        fit = 0.6 + apt / 125
        # 3. a palya jellege illik-e a lo futasstilusahoz
        style = RC.infer_running_style(horse.genetics)
        bias = RC.STYLE_TRACK_BONUS.get(r.style_bias, {}).get(style, 1.0)
        # 4. a dij csak SULYOZO, nem donto
        return same * fit * bias * (1.0 + r.purse / 200000)

    options.sort(key=score, reverse=True)
    entered = []
    for r in options:
        if len(entered) + already >= max_entries:
            break
        res = calendar.enter(horse, owner_id, r.race_id)
        if res['ok']:
            entered.append(r)
    return entered


def auto_enter_all(world, calendar, owner_id):
    """Minden 'auto' politikaju lo nevezese."""
    out = []
    for h in world.owned_by(owner_id, 'racer'):
        for r in auto_enter(world, calendar, h, owner_id):
            out.append((h, r))
    return out
